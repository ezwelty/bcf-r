...
#'
#' @examples
#' parse_sizes(c("1", "12 oz", "2 x 12 oz", "2 x 1 lb", "2 gal"))
...
parse_categories <- function(categories) {
  ...
  replacements <- c(
    Grain = "Grains",
    `Non Food` = "Non-Food",
    `Ecofarm Rice` = "Eco-Farm Rice",
    Spice = "Spices",
    Sweetners = "Sweeteners",
    Trailmix = "Trail Mixes"
  )
  replace <- categories %in% names(replacements)
  categories[replace] %<>%
    replacements[.]
  categories
}
